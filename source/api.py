...
class DeepSearchConfig:
    ...
    def from_env(cls) -> "DeepSearchConfig":
        ...
        base_url = os.getenv("DEEPSEARCH_BASE_URL", cls.base_url)
        model = os.getenv("DEEPSEARCH_MODEL", cls.model)
        timeout = float(os.getenv("DEEPSEARCH_TIMEOUT", cls.timeout))
...
class DeepSearchTransport:
    ...
    def from_config(cls, config: DeepSearchConfig, **kwargs: Any) -> "DeepSearchTransport":
        merged = {"api_key": config.api_key, "base_url": config.base_url, "model": config.model, "timeout": config.timeout}
    ...
    def invoke_tool(self, tool_name: str, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            raise DeepSearchAPIError(f"DeepSearch API 返回错误状态: {exc.response.status_code}") from exc
